...
def weaponDetection():
    rf = Roboflow(api_key="Dj0mFTI5QZWxLsVyuMG3")
    project = rf.workspace().project("weapondetection-pmx4d")
    ...
    model.predict("your_image.jpg", confidence=40,
        ...
        if not success:
            ...
            if len(faces) > 0:
                ...
                cv2.putText(img, "Time: " + str(Min) + " Mins " + str(Sec) + " Sec ",
                            (0, img.shape[0] - 30), cv2.FONT_HERSHEY_TRIPLEX, 0.5,  (0, 0, 255), 1)
                cv2.putText(img, "Number of faces detected: " + str(
                    faces.shape[0]), (0, img.shape[0] - 10), cv2.FONT_HERSHEY_TRIPLEX, 0.5,  (0, 0, 255), 1)
            ...
            if len(faces) == 0:
                ...
                print('No face detected')
                cv2.putText(img, "No face detected ", (0,
                            img.shape[0] - 10), cv2.FONT_HERSHEY_TRIPLEX, 0.5,  (0, 0, 255), 1)
                Sec = 0
                Min = 0
